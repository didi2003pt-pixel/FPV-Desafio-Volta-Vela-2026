...
export default async function AdminPage() {
  const [boats, stages, users, markets, predictions, flags] = await Promise.all([
    prisma.boat.count(),
    prisma.stage.count(),
    prisma.user.count(),
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-8">
      <p className="text-sm font-black uppercase tracking-[0.15em] text-brand-red">Fase 2 · Jogo</p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Card><p className="text-sm text-slate-500">Mercados</p><p className="text-4xl font-black">{markets}</p></Card>
